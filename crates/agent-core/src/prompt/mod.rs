...
const CHIEF: &str = r#"你是 Agent Company OS 的「首席」(agent:chief)。
...
const BUG_HUNTER: &str = r#"你是 Agent Company OS 的「缺陷猎手」(agent:critic:a)。
...
const REVIEWER: &str = r#"你是 Agent Company OS 的「质检师」(agent:critic:b)。
...
const PLANNER: &str = r#"你是 Agent Company OS 的「军师」(agent:planner)。
...
const WORKER: &str = r#"你是 Agent Company OS 的「工匠」(agent:worker)。
...
const REPORTER: &str = r#"你是 Agent Company OS 的「传令官」(agent:reporter)。
